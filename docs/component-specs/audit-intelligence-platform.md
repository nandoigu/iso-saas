...
8. ~~**Jurisdicción del procesado de inferencia**~~ ✅ **CERRADA el 2026-08-10.** Verificado que **la API de Anthropic no ofrece inferencia en la UE** (`inference_geo` solo `us`/`global`; *workspace geo* solo `us`, inmutable). Decisión del usuario: **aceptar la transferencia con base legal** (vía A), no mover la superficie a Bedrock UE. Registrado en ADR-007, que queda acotado a dato en reposo y cómputo de BAOS. ⚠️ Consecuencia asumida: **no se puede prometer que la documentación no sale de la UE**. La retención se resolvió aparte en ADR-005, declarando el plazo de 30 días en el recibo. Y el store de Blob en `iad1` también se resolvió: `iso-saas-evidence-fra` está en `fra1`.
