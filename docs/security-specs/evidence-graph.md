...
⚠️ **El acceso privado del store es un control de infraestructura, no de código** — decisión registrada en **ADR-014**, que recoge también la alternativa que lo trasladaría al código (`handleUploadPresigned`) y su disparador de reconsideración: el primer documento real de cliente o la construcción de la UI de subida, lo que ocurra antes. El handshake de client upload deja el `access` en manos del cliente; lo que impide publicar una evidencia es que el store esté configurado como privado (`Cannot use public access on a private store`, verificado el 2026-08-16). **Si algún día se sustituye el store, comprobar esa propiedad antes de dar por buena la protección** — el código no la puede garantizar por sí solo. La sonda `scripts/probar-blob-fase-d.mjs` la verifica contra el store real.
